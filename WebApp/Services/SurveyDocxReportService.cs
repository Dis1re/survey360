...
public class SurveyDocxReportService(ApplicationDbContext context, ILogger<SurveyDocxReportService> logger)
{
    ...
    private sealed record SurveyReportContext(
        Survey Survey,
        List<Question> Questions,
        List<Answer> Answers,
        Dictionary<int, User> UsersById,
        Dictionary<(int QuestionId, int UserId, int TargetId), Answer> AnswersByQuestionReviewerTarget,
        HashSet<(int ReviewerId, int TargetId)> AssignedPairs,
        List<int> TargetIds);
    ...
    public async Task<(byte[] Bytes, string FileName)?> BuildReportAsync(int surveyId, CancellationToken ct)
    {
        try
        {
            ...
            using (var document = WordprocessingDocument.Create(stream, WordprocessingDocumentType.Document, true))
            {
                ...
                foreach (var element in BuildTitleElements(report.Survey, "Результаты опроса"))
                    body.Append(element);

                foreach (var targetId in report.TargetIds)
                {
                    var targetName = report.UsersById.GetValueOrDefault(targetId)?.Name ?? $"Пользователь #{targetId}";
                    body.Append(CreateSectionHeading($"Оцениваемый: {targetName}"));

                    var reviewerIds = report.Answers
                        .Where(a => a.TargetId == targetId)
                        .Select(a => a.UserId)
                        .Distinct()
                        .Where(reviewerId =>
                            report.AssignedPairs.Count == 0 ||
                            report.AssignedPairs.Contains((reviewerId, targetId)))
                        .OrderBy(reviewerId => report.UsersById.GetValueOrDefault(reviewerId)?.Name ?? reviewerId.ToString())
                        .ToList();

                    foreach (var reviewerId in reviewerIds)
                    {
                        var reviewerName = report.UsersById.GetValueOrDefault(reviewerId)?.Name ?? $"Пользователь #{reviewerId}";
                        body.Append(CreateSubsectionHeading($"Респондент: {reviewerName}"));

                        var questionIndex = 1;
                        foreach (var question in report.Questions)
                        {
                            report.AnswersByQuestionReviewerTarget.TryGetValue(
                ...
                body.Append(CreateClosingBlock(report.Survey.Name));
                body.Append(CreateSectionProperties(headerPartId));
                mainPart.Document.Save();
            }

            var fileName = $"{SanitizeFileName(report.Survey.Name)}-результаты.docx";
        ...
        try
        {
            ...
            using (var document = WordprocessingDocument.Create(stream, WordprocessingDocumentType.Document, true))
            {
                ...
                foreach (var question in report.Questions)
                {
                    ...
                    var questionAnswers = report.Answers
                        .Where(a => a.QuestionId == question.Id)
                        .Where(a =>
                            report.AssignedPairs.Count == 0 ||
                            report.AssignedPairs.Contains((a.UserId, a.TargetId)))
                        .OrderBy(a => report.UsersById.GetValueOrDefault(a.TargetId)?.Name ?? a.TargetId.ToString())
                        .ThenBy(a => report.UsersById.GetValueOrDefault(a.UserId)?.Name ?? a.UserId.ToString())
                        .ToList();

                    if (questionAnswers.Count == 0)
                    {
                        body.Append(CreateParagraph("Нет ответов", color: MutedColor));
                    }
                    else
                    {
            ...
            var fileName = $"{SanitizeFileName(report.Survey.Name)}-результаты-по-вопросам.docx";
            return (stream.ToArray(), fileName);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to build by-question DOCX report for survey {SurveyId}", surveyId);
            throw;
        }
    }
    ...
    private async Task<SurveyReportContext?> LoadReportContextAsync(int surveyId, CancellationToken ct)
    {
        ...
        return new SurveyReportContext(
            survey,
            questions,
            answers,
            usersById,
            answersByQuestionReviewerTarget,
            assignedPairs,
            targetIds);
    }
    ...
    private IEnumerable<OpenXmlElement> BuildTitleElements(Survey survey, string subtitle)
    {
        yield return CreateCenteredParagraph(survey.Name, bold: true, color: BrandColor, sizeHalfPoints: 32);
        yield return CreateCenteredParagraph(subtitle, bold: true, color: TextColor, sizeHalfPoints: 22);
