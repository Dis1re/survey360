  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto">
        {counts.map((c, i) => (
          <div key={i} className="flex flex-col items-center gap-0.5 shrink-0" style={{ minWidth: counts.length > 12 ? '1.75rem' : '2.25rem' }}>
  ...
  return (
    <div>
      ...
      <div className="space-y-1">
        ...
        {qAnswers.map((a) => (
          <div key={a.id} className="flex flex-wrap gap-x-2 gap-y-0.5 text-xs bg-gray-50 dark:bg-[#161a22] rounded-lg px-3 py-2">
            ...
            <span className="text-gray-700 dark:text-gray-200 break-words min-w-0 flex-1">: {a.text || '—'}</span>
          </div>
        ))}
      </div>
    </div>
