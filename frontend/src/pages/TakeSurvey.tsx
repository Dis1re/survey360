...
function QuestionInput({
        ...
        {values.map((n) => (
          <button
            ...
            className={`soft-press flex-1 min-w-[44px] py-3 rounded-xl border text-sm font-semibold ${
              ...
              selected === n
                ? 'border-[#FF8600] bg-[#FF8600] text-white'
                : readOnly
                  ? 'border-gray-200 dark:border-[#3a4250] bg-gray-50 dark:bg-[#1e222e] text-gray-400 dark:text-gray-300'
                  : 'border-gray-200 dark:border-[#3a4250] text-gray-600 dark:text-gray-200 hover:border-[#FF8600] dark:hover:border-[#FF8600]'
            }`}
          >
            {n}
          </button>
    ...
    if (options.length === 0) {
      return (
        <input
          ...
          className={`w-full border border-gray-200 dark:border-[#3a4250] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 ${
            readOnly ? 'bg-gray-50 dark:bg-[#1e222e] text-gray-600 dark:text-gray-300 cursor-default' : ''
          }`}
        />
      )
    }
        ...
        {options.map((opt) => (
          <label
            ...
            className={`soft-lift flex items-start gap-3 p-3 rounded-xl border ${
              ...
              value === String(opt.value) ? 'border-blue-500 bg-blue-50 dark:bg-blue-500/10' : 'border-gray-100 dark:border-[#3a4250] hover:bg-gray-50 dark:hover:bg-[#262d3a]'
            } ${readOnly ? 'opacity-80' : ''}`}
          >
            <input
              type="radio"
  ...
  return (
    <textarea
      ...
      className={`w-full border border-gray-200 dark:border-[#3a4250] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 resize-none ${
        readOnly ? 'bg-gray-50 dark:bg-[#1e222e] text-gray-600 dark:text-gray-300 cursor-default' : ''
      }`}
    />
  )
